...
export default function StatRegUserList({
  label,
  required,
  valueProps,
  placeholder,
  ...
  if (data) {
    for (let item of data) {
      defaultData = [
        ...defaultData,
        {
          label: `${item?.executive?.full_name?.split(' ')[0]} (${
            item?.executive?.contact_number
          })`,
          value: `${item?.executive?.id}`,
        },
      ]
    }
  }
  ...
  const [stagreg, setStatReg] = useState({})
  ...
  const handelSelectItem = item => {
    ...
    setSelectItem(item)
    // valueProps(item)
    console.log('itemOfStat', item)
    let split_string = item?.value?.split(/(\d+)/)
    if (setUserInfo) {
      setUserInfo(split_string[1])
    }
    setOpenList(prv => !prv)
  }
  ...
  //       ]
  //       label: {
  //         item?.executive?.full_name?.split(' ')[0]
  //       }
  //       {
  //         ;(' (')
  //       }
    ...
    <View style={styles.container}>
      {/* <View style={styles.labelContainerStyle}>
        <Text preset="h5" style={[styles.label, labelStyle]}>
          {label !== undefined ? label : languageState?.registeredUsersTitle}
        </Text>
      </View> */}
      ...
      {/* <>
        ...
        >
          ...
          >
            {userInfo?.executive?.full_name === undefined ? (
              <Text style={[styles.inputLabelStyle]} color={'#979797'}>
                {'Select an user'}
              </Text>
            ) : (
              <Text style={[styles.inputLabelStyle]}>
                {userInfo?.executive?.full_name?.split(' ')[0]}
                {' ('}
                {userInfo?.executive?.contact_number}
                {')'}
              </Text>
            )}
