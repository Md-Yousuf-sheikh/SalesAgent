  ...
  const handelSameAsOwnerPresentAddress = async (
    ...
    try {
      if (isChecked) {
        ...
        const personalInformationInfo = personalInformation?.present_postal_code
          ? personalInformation
          : authCustomer;

        // auth customer
        if (authCustomer !== undefined) {
          // set data
          handelChange(
            personalInformationInfo?.present_postal_code,
            "postal_code",
            inputIndex
          );
          handelChange(
            personalInformationInfo?.present_thana,
            "thana",
            inputIndex
          );
          handelChange(
            personalInformationInfo?.present_district,
            "district",
            inputIndex
          );
          handelChange(
            personalInformationInfo?.present_country,
            "country",
            inputIndex
          );
          handelChange(
            personalInformationInfo?.present_address,
            "address",
            inputIndex
          );
        }
    ...
    <View>
      ...
      {member &&
        ...
        currentPolicy?.number_of_insured > 1 && (
          <>
            <NomineeAddButton
              title={language?.memberTitle}
              onPress={addMoreMemberInput}
              disabled={
                nomineeInputs?.length >= (currentPolicy?.number_of_insured || 1)
              }
            />
              ...
              <>
                {memberInputs?.length > 0 &&
                  memberInputs?.map((memberInput, inputIndex) => (
                    <AccordionItem
                      title={`${language?.memberTitle} (${inputIndex + 1})`}
                    >
                      {/* filed */}
                                ...
                                <View>
                                  ...
                                  <SinglePicker
                                    ...
                                    disabled={
                                      ...
                                      memberInputs[inputIndex]["postal_code"]
                                    }
                                  />
                                  <FormErrorMessage
                                    message={
