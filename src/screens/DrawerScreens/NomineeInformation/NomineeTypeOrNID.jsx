...
export default function NomineeTypeOrNID({
  identity,
  title,
  nomineeData,
  personalInfo,
  ...
  //     datas => datas?.field_name === 'nominee_identification_type'
  //   )?.[0]
  //   memberIdentity = purchaseFormDetails?.Member_Information?.filter(
  //     datas => datas?.field_name === 'member_identification_type'
  //   )?.[0]
  // }

  // useEffect(() => {
  //   if (identity?.field_name === 'identity_type' && userInfo?.identity_type) {
  ...
  // useEffect(() => {
  //   if (personalInfo) {
  //     if (personalInfo?.attachment_front) {
  //       setNIDFrontSide(personalInfo?.frontLink)
  //       setProvideNIDLater(false)
  //     }
  //     if (personalInfo?.attachment_back) {
  //       setNIDBackSide(personalInfo?.backLink)
  //       setProvideNIDLater(false)
  //     }
  //   }
  //   console.log('personalInfoofNid', personalInfo)
  // }, [personalInfo])
  ...
  useEffect(() => {
    async function getCustomerIdentity() {
      try {
        const data = {
          user_id: userInfo?.id,
          identity_type: personalInfo?.identity_type,
        };
  ...
  return (
    <>
      ...
      <View>
        ...
        {/* Input filed */}
        {(identity?.field_type === "select" ||
          identity?.field_type === "hidden") && (
          <CustomSinglPicker
            data={identity?.field_options}
            label={identity?.field_title}
            placeholder={identity?.field_placeholder}
            valueProps={setIdentityType}
            setPersonalInfo={setPersonalInfo}
            personalInfo={personalInfo}
            inputText={identity?.field_name}
            errorCheck={errorCheck}
            required={identity?.field_required}
            inputIndex={inputIndex}
            disabled={
              identity?.field_type === "hidden" || disableCheck ? true : false
            }
            item={identity}
            userInfo={userInfo}
          />
        ...
        {identityNumber?.field_type === "text" && (
          <FormInputText
            label={
              personalInfo[identity?.field_name]
                ? personalInfo[identity?.field_name]
                : identityNumber?.field_title
            }
            placeholder={
              personalInfo[identity?.field_name]
                ? personalInfo[identity?.field_name]
                : identityNumber?.field_placeholder
            }
            markHide={true}
            value={personalInfo[identityNumber?.field_name]}
            // setValue={setNidNumber}
            // error={filedError}
            onChangeText={(text) =>
