...
import { useLazyGetPremiumDuesQuery } from "../../redux/features/customer/customerApiSlice";
...
export default function PremiumDue() {
  ...
  // API
  const [getPremiumDueData, { isLoading }] = useLazyGetPremiumDuesQuery();
  ...
  async function handleDateFilter() {
    if (dateTo && dateFrom && dateFrom !== undefined && dateTo !== undefined) {
      try {
        ...
        const res = await getPremiumDueData([paramData]).unwrap();
        setFilterDatas(res?.data);
        setFilterUserData(res?.data?.premiumDues);
  ...
  const searchFilterFunction = (text) => {
    ...
    if (text) {
      // Inserted
      const newData = filterDatas?.premiumDues?.filter(function (item) {
        // Applying
  ...
  tarData = {
    ...
    count: parseInt(filterDatas?.due),
    title: "Due",
  };
  //
  // renderLoader
  ...
  return (
    <>
      ...
      />
      <>
        <DrawerHeader title={language.premiumDueTitle} />
        <Background
          ph={10}
          pv={10}
          onRefresh={() => {
          ...
          >
            <View style={styles.headerContainer}>
              <Text style={styles.textStyle} preset="LM">
                {language.totalPremiumDueTitle}:{language?.bdt}{" "}
                {ToEnNum(CurrencyFormat(parseInt(filterDatas?.due)), code)}
              </Text>
            </View>
            <PaymentCalCard
              ...
              secondTitle={language.pendingAmountTitle}
              firstAmount={parseInt(filterDatas?.total)}
              secondAmount={parseInt(filterDatas?.due)}
              target={tarData}
              type={language?.bdt}
              progressTitle={"Due"}
            />
              ...
              return (
                <CollapsibleListCard
                  ...
                  headList={[
                    ...
                    {
                      title: "Email",
                      value: `${language?.bdt} ${ToEnNum(
                        CurrencyFormat(item?.policy?.premium_amount),
                        code
                      )}`,
                      width: 35,
                    },
