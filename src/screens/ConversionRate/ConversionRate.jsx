...
import { useLazyGetFilterAgentsLeadQuery } from "../../redux/features/customer/customerApiSlice";
...
export default function ConversionRate() {
  ...
  const [getFilterAgentsLead, { isLoading }] =
  ...
  async function handleDateFilter() {
    if (dateTo && dateFrom && dateFrom !== undefined && dateTo !== undefined) {
      try {
        ...
        const res = await getFilterAgentsLead([paramData]).unwrap();
        //
        let data = res?.leads?.filter((item) => item?.status === "Converted");
        setFilterDatas(res);
        setFilterUserData(data);
        let graphData = [
          {
            title: "Conversion",
            count: `${(parseInt(res?.convert) / parseInt(res?.total)) * 100}%`,
            value: `${(parseInt(res?.convert) / parseInt(res?.total)) * 100}`,
          },
        ];
        setGraphVal(graphData);
  ...
  async function pageFilter() {
    try {
      ...
      if (currentPage == 1) {
        ...
        setDataNew((prevData) => [...prevData, ...data]);
        setFilterUserData((prevData) => [...prevData, ...data]);
      }
      let graphData = [
        {
          title: "Conversion",
  ...
  const searchFilterFunction = (text) => {
    if (text) {
      // Inserted
      let data = filterDatas?.leads?.filter(
  ...
  let tarData;
  tarData = {
    total: parseInt(filterDatas?.total),
    count: parseInt(filterDatas?.convert),
    title: "Converted",
  };

  let nanCon = filterDatas?.convert / filterDatas?.total;
  // renderLoader
  const renderLoader = () => {
    return filterDatas &&
  ...
  return (
    <>
      ...
      />
      <>
        <DrawerHeader title={language.conversionRateHeader} />
        <Background
          ph={10}
          pv={10}
          onRefresh={() => {
          ...
          >
            <View style={styles.headerContainer}>
              <Text style={styles.textStyle} preset="LM">
                {language.totalConversionRateTitle}:{" "}
                {Math.round((nanCon ? nanCon : 0) * 100)}
                {"%"}
              </Text>
            </View>
            <PaymentCalCard
              ...
              firstTitle={language.totalLeadTitle}
              secondTitle={language.convertedCustomerTitle}
              firstAmount={parseInt(filterDatas?.total)}
              secondAmount={parseInt(filterDatas?.convert)}
              target={tarData}
            />
          </View>
          {/* search  */}
